...
const HomeTimeline = () => {
  ...
  const parseTurkishDate = (dateStr) => {
    try {
      ...
      const month = parts[1];
      // Her tarih için 2025 yılını kullan
      const year = '2025';
  ...
  const formattedSeminars = seminerler.map(seminer => {
    let dateStr, timeStr, formattedSeminer;
    
    if (seminer.detailData) {
      dateStr = seminer.detailData.sessions[0].startDate.replace(/\s+20\d{2}$/, '');
      dateStr = `${dateStr} 2025`;
      timeStr = seminer.detailData.sessions[0].endDate;
      ...
      dateStr = `${seminer.date} 2025`;
      timeStr = seminer.time;
      formattedSeminer = {
        ...seminer,
        firstSession: {
          startDate: dateStr,
    ...
    return {
      ...formattedSeminer,
      dateObj: seminerDate,
      isPast: seminerDate < today
    };
  }).sort((a, b) => {
    const dateComparison = a.dateObj - b.dateObj;
    if (dateComparison === 0) {
      const timeA = timeToMinutes(a.firstSession.endDate);
      const timeB = timeToMinutes(b.firstSession.endDate);
      return timeA - timeB;
    }
  ...
  const futureEvents = formattedSeminars.filter(seminer => {
    const eventDate = new Date(seminer.dateObj);
